 ...
 * */
static int 
gen_get_options(int argc, char *argv[])
{
    int opt = 0;
...
/* callback function needed by libnids */
void
...
/*
 * Description: Initialize streamGen
 *
 * */
void 
gen_init(int argc, char *argv[]){
    int ret;
    int i;
    ...
    fprintf(stdout, "NUMA info, socket id: %d, socket id for network device: %d\n", rte_socket_id(), rte_eth_dev_socket_id(0));
...
/*
 * Description:  Main thread: import pcap --> slice --> slimulate TCP stream
 * 
 * */
void
gen_run(void)
{
    ...
    /* import pcap file and store stream data*/
    printf("\nImport source data from pcap file...\n");
    nids_run();
